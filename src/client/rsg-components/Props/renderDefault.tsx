...
export default function renderDefault(
  ...
  if (prop.defaultValue) {
    const defaultValueString = showSpaces(
      unquote(String(prop.defaultValue.value))
    );
    if (prop.type || prop.flowType) {
      const propName = prop.type
        ? prop.type.name
        : prop.flowType && prop.flowType.type;

      if (defaultValueBlacklist.indexOf(prop.defaultValue.value) > -1) {
        return <div>{defaultValueString}</div>;
        ...
        try {
          // We eval source div to be able to format the defaultProp here. This
          // can be considered safe, as it is the source div that is evaled,
